...
def get_true_jid(jid):
   ...
   if stripped_jid in GROUPCHATS:
      if resource in GROUPCHATS[stripped_jid]:
         true_jid = string.split(str(GROUPCHATS[stripped_jid][resource]['jid']), '/', 1)[0]
      else:
...
def get_bot_nick(groupchat):
   if check_file(file='chatrooms.list'):
      ...
      if groupchat in gchdb and gchdb[groupchat]['nick']:
...
def get_gch_info(gch, info):
   if check_file(file='chatrooms.list'):
      gchdb = eval(read_file(GROUPCHAT_CACHE_FILE))
      if gch in gchdb:   return gchdb[gch].get(info)
      else:   return None
   else:
      print('Error adding groupchat to groupchats list file!')   
...
def timeElapsed(time):
   ...
   rep = '%d sec' % (round(seconds))
   if time>60: rep = '%d min %s' % (minutes, rep)
   if time>3600: rep = '%d hour(s) %s' % (hours, rep)
   if time>86400: rep = '%d day(s) %s' % (days, rep)
   if time>2592000: rep = '%d month(s) %s' % (months, rep)
   return rep
...
def change_access_temp(gch, source, level=0):
   ...
   try:
      ...
      level = 0
   if gch not in ACCBYCONF:
      ACCBYCONF[gch] = gch
      ACCBYCONF[gch] = {}
   if jid not in ACCBYCONF[gch]:
...
def change_access_perm(gch, source, level=None):
   ...
   if gch not in temp_access:
      temp_access[gch] = gch
      temp_access[gch] = {}
   if jid not in temp_access[gch]:
...
def user_level(source, gch):
   ...
   if gch in ACCBYCONFFILE:
      if jid in ACCBYCONFFILE[gch]:
         return ACCBYCONFFILE[gch][jid]
   if gch in ACCBYCONF:
      if jid in ACCBYCONF[gch]:
...
def reply(ltype, source, body):
   if not isinstance(body, str):
      body = body.decode('utf8', 'replace')
   if source[1] in list(GCHCFGS.keys()) and GCHCFGS[source[1]]['afools']==1:
...
def isadmin(jid):
   ...
   if stripped_jid in ADMINS:
      return 1
   elif stripped_jid in GROUPCHATS:
      if resource in GROUPCHATS[stripped_jid]:
...
def messageHnd(con, msg):
   ...
   if msgtype == 'groupchat':
      mtype='public'
      if fromjid.getStripped() in GROUPCHATS and fromjid.getResource() in GROUPCHATS[fromjid.getStripped()]:
   ...
   if command in COMMANDS:
      if fromjid.getStripped() in COMMOFF and command in COMMOFF[fromjid.getStripped()]:
         ...
         call_command_handlers(command, mtype, [fromjid, fromjid.getStripped(), fromjid.getResource()], str(parameters), rcmd)
         INFO['cmd'] += 1
         LAST['t'] = time.time()
         LAST['c'] = command
         if fromjid.getStripped() in GROUPCHATS:      
            if GCHCFGS[fromjid.getStripped()]['autoaway']==1:
               ...
               LAST['gch'][fromjid.getStripped()]['thr']=threading.Timer(600,change_bot_status,(fromjid.getStripped(), 'auto away due to being idle '+time.strftime('%d.%m.%Y@%H:%M:%S GMT', time.gmtime()), 'away',1))
...
def presenceHnd(con, prs):
   ...
   if groupchat in GROUPCHATS:
      if ptype == 'unavailable':
         ...
         if not jid:
            time.sleep(2)
            msg(groupchat, 'dammit!! you didnt add my jid to admin list and thus i cannot function, i will leave now ,bye.')
            time.sleep(1)
            leave_groupchat(groupchat, 'Absence of the rights of a moderator')        
            return
         else:
         ...
         if ecode:
            if ecode == '409':
               ...
               leave_groupchat(groupchat, 'got %s error code' % str(ecode))
            elif ecode == '503':
   ...
   if check_file(file='chatrooms.list'):
      ...
      for groupchat in groupchats:
         ...
         with smph:
            ...
            if groupchat in list(LAST['gch'].keys()):
               if GCHCFGS[groupchat]['autoaway']==1:
                  LAST['gch'][groupchat]['thr']=threading.Timer(600,change_bot_status,(groupchat, 'auto away due to being idle '+time.strftime('%d.%m.%Y@%H:%M:%S GMT', time.gmtime()), 'away',1))
...
if __name__ == "__main__":
   try:
      ...
      prs.setStatus('got Ctrl-C -> shutdown')
      JCON.send(prs)
      time.sleep(2)
      print('DISCONNECTED')
      print('\n...---===BOT STOPPED===---...\n')
